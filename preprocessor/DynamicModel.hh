...
private:
  ...
  typedef vector<pair<int, pair<string, pair<string, string > > > >dmmcalib_t;
  dmmcalib_t dmmCalibration;
  // symb_id -> (multinomial process name, regime number)
  vector<pair<int, pair<string, string > > >dmmPrior;
...
public:
  ...
  */
  ...
  void setDmmLatentVarInfo(vector<pair<int, pair<string, pair<string, string > > > >&calibration,
